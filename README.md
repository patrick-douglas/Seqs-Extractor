...
Or just open Seqs-Extractor from Linux main menu  **NOTE: There is no desktop or launcher icon for MAC OSX** 
------------------------------------------------------------------------------------------------------------
...
that contains all available protein sequences of mouse obtained in The universal protein resource (UniProt) by searching for "mouse" in the search field and filtering the search to show only 
...
With Seqs-Extractor you can run BLASTx search and easily generate the result file in all available BLAST+ formats, in this example we used the option 6 to generate a tabular
file, so the Seqs-Extractor run will result in the file **blastx-evalue_1e-3.outfmt6** (**NOTE: YOU CAN FIND THIS FILE IN THE *Opt_1_2_3_BLAST_results* folder)

**M.musculus_100%_Extracted_seqs**
----------------------------------
Here we used the Seqs-Extractor to extract from our query file (**M.musculus_NCBI_entire_genome.fasta**) only the top hits of 100% of similarity that we get from the previous 
...
You can also run Seqs-Extractor to perform a BLAST search and extract sequences.
...
The file will be generated in .FASTA file and will be named as **extracted-seqs.fasta**. It will be stored in your working folder.
...
By default, you will use default MISA parameters to search for microssatelites. You can modify these parameters if you prefer to (see the section **8** of this manual).
The final screen will indicate the name of the files (MISA results files and the .FASTA file with the sequences extracted). The sequences in .FASTA file generated will be named with 
the ID of the sequences obtained from the input file (for use in Trinity Differential Expression pipeline see the section **6** of this manual). It will be stored in the same directory you are working.


#**6.** Running MISA and extracting sequences maintaining the additional information in the title 
-----------------------------------------------------------------------------------------------

For users who needs to use extracted sequences of MISA search in Trinity RNA-Seq differential expression pipeline (Grabherr, M. G et al 2011), Seqs-Extractor allows a running-extraction that generates a .FASTA file whose sequences are named exactly as it was in the input file, maintaining additional information as length and path of the sequences in its titles. 
...
The final screen will indicate the name of the files (MISA results files and the .FASTA file with the sequences extracted). The sequences in .FASTA file generated will be named with 
the full title of the sequence obtained from the input file. It will be stored in the same directory you are working.
...
**M.musculus_NCBI_entire_genome.fasta.misa** (This file is the MISA results performed over **M.musculus_NCBI_entire_genome.fasta**)

**M.musculus_NCBI_entire_genome.fasta.statistics** (This file is the MISA statistics performed over **M.musculus_NCBI_entire_genome.fasta**)

**M.musculus_NCBI_entire_genome.fasta.MISA_extracted_seqs.fasta** (This file contains only the MISA positive positive sequences extracted by Seqs-Extractor from **M.musculus_NCBI_entire_genome.fasta**)
...
The .FASTA file with only the sequences indicated by the text file will be stored in the same directory you are working. It will be named as 
**name_of_text_file.name_of_the_input_fasta_file.extracted_seqs.fasta**.

#RUNNING AN EXAMPLE FOR EXTRACTION FROM ANY TEXT FILE

The folder **Opt_7_Extract_from_txt_file** contain two files, the first **example_list.txt** is an example of a list to use in the extraction process 
and **example_list.txt.M.musculus_NCBI_entire_genome.fasta.extracted_seqs.fasta** is the result of extraction of sequences from **M.musculus_NCBI_entire_genome.fasta** using Seqs-Extractor. 



#**8.** Customize **misa.ini** file to modify parameters of identification of microssatellites
---------------------------------------------------------------------------------------------

This option will open the **misa.ini** file in a text editor to allow you to modify the parameters of identification of microssatellites in the sequences.
...
value of repeat of that unit to be considerated as a microssatelite (example: 2-6 indicates that MISA will considerate as microssatelite a pair of nucleotides that repeats at least six times in tandem).

- The second line of this file indicates the maximum number of nucleotides between two microssatellites, refering to compound microssatellites (example: 100).
...
#**9.**Frequent asked questions and known bugs
----------------------------------------------

	1. Error: misa.pl not found or **misa.pl permission denied: Check if you are running the Seqs-Extractor in the same hard disk of your Linux system as well
